...
export async function createOrder(input: PlaceOrderInput): Promise<PlaceOrderResult> {
  ...
  // 1. First attempt: call RPC `create_order` if defined in Supabase
  try {
    ...
    const { data: rpcData, error: rpcError } = await supabase.rpc('create_order', {
      ...
      p_payment<dyad-write path="src/services/api.ts" description="Completing api.ts with validated createOrder and submitOrderPayment methods">
import { supabase } from '@/lib/supabase';
import type {
  Product, Category, PaymentMethod, Coupon, ShippingConfig, StoreConfig,
  Order, OrderItem, Payment, Address, Profile, Review,
...
export async function createOrder(input: PlaceOrderInput): Promise<PlaceOrderResult> {
  ...
  // 1. Fetch & validate payment method
  ...
  // 3. Re-verify products, stock and calculate recalculating amounts directly from DB
  ...
  for (const item of items) {
    ...
    let originalPrice = product.original_price ? Number(product.original_price) : null;
  ...
  if (itemsError) {
    // If inserting items fails, clean up the orphaned order
  ...
  if (payError) {
    console.warn('Initial payment record creation notice:', payError.message);
  }
...
export async function submitOrderPayment(
  ...
  if (existingPayment) {
    await supabase.from('payment_status_history').insert({
      ...
      note: reference ? `Customer reference: ${reference}` : 'Submitted by customer',
    });
  }
}
